...
static const char *tag = "BTDrive"; // Device name(as seen by connected devices)
        ...
        case 0x02: // Obex PUT(Send file)
            ...
            for (int i = 14; i < len - 2;)
            {
                ...
                else if (packet[i] == 0x01)
                {
                    ...
                    //file_name = malloc(head_len - 3);
		    strcpy(file_name, "/a/");
		    char *fptr = file_name + strlen(file_name);
		    ...
		    *fptr = 0;	//mark end
			...
			received_bytes++;
                        // printf("%c", packet[j]);
			//if ( ! (fwrite(&packet[j], 1, 1, fd)) )
			if ( (fputc(packet[j], fd) == EOF) )
			    perror("fwrite");
                    }
                    i += head_len;
                }
